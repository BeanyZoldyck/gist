...
def get_relevant_context(rewritten_input, vault_embeddings, vault_content, top_k=9):
    if vault_embeddings.nelement() == 0:  # Check if the tensor has any elements
        return []
    # Encode the rewritten input
    input_embedding = ollama.embeddings(model='qwen3-embedding', prompt=rewritten_input)["embedding"]
...
for content in vault_content:
    response = ollama.embeddings(model='qwen3-embedding', prompt=content)
